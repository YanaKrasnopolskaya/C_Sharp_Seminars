...
// int[,] MatrixRowColDelByCoords2(int[,] matrix, int rowTodel, int colTodel)
// {
//     int[,] resMatrix = new int[matrix.GetLength(0) - 1, matrix.GetLength(1) - 1];

//     for (int i = 0, row = 0; i < resMatrix.GetLength(0); i++, row++)
//     {
//         if (i == rowTodel) row++;
//         for (int j = 0, col = 0; j < resMatrix.GetLength(1); j++, col++)
//         {
//             if (j == colTodel) col++;
//             resMatrix[i, j] = matrix[row, col];
//         }
...
// int[] coords = FindLowestValueRowCol(rndMtrx);
// Console.WriteLine($"row {coords[0]} col {coords[1]} min val = {rndMtrx[coords[0], coords[1]]}");
// int[,] resMtrx = MatrixRowColDelByCoords2(rndMtrx, coords[0], coords[1]);
// PrintMatrix(resMtrx);